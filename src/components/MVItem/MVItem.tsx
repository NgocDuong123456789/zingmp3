...
const MVItem = ({ encodeId, title, thumbnail, link, artistsNames, thumbnailM }: MVType) => {
  ...
  return (
    <div className='grid grid-cols-1 pb-5 cursor-pointer' 
    ...
    >
      <div className='w-full relative overflow-hidden hover:rounded-2xl'>
        ...
        {isHover && ( <div className='top-0 left-0 right-0 bottom-0 absolute flex items-center justify-center bg-[rgba(0,0,0,0.3)] z-10'>
          <span className='border-2 p-3 rounded-full'><Icons.BsFillPlayFill size={30} /></span>
        </div>)}
      </div>
